#!/usr/bin/env python3
"""
OP Kortteli & Antell Vallila — Lounas RSS-generaattori v8
- Pannu:       lounastaja.app RSS-feedi
- Kulho, Uuni: suora HTTP-haku Wix-sivulta, menu comp-m7ngbax61 -elementistä
...
def html_to_text(inner_html: str) -> str:
    """Muuntaa HTML-snippetin puhtaaksi tekstiksi."""
...
# ── Kulho ja Uuni: staattinen Wix HTML ───────────────────────────────────────

def scrape_soupster_html(url: str) -> str:
    """
    Wixin SSR-HTML sisältää menun suoraan comp-m7ngbax61-elementissä.
    Elementti päättyy Wixin <!--/$--> -kommenttimerkintään,
    joka on luotettava rajamerkki sisäkkäisistä diveistä huolimatta.
    """
    html = http_get(url)

    # Etsi viimeinen comp-m7ngbax* -elementti (se on yleensä menuosio)
    # Haetaan kaikki vastaavat, valitaan pisin
    matches = re.findall(
        r'id="comp-m7ngbax\w+"[^>]*>(.*?)<!--/\$-->',
        html, re.DOTALL
    )
    ...
    if not matches:
        return "(Menu-elementtiä ei löydy HTML:stä)"

    # Valitaan pisin lohko — se on todennäköisimmin ruokalista
    best_html = max(matches, key=len)
    text = html_to_text(best_html)

    if not text:
        return "(Menu-elementti tyhjä)"
    return text
