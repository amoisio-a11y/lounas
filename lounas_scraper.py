#!/usr/bin/env python3
"""
OP Kortteli & Antell Vallila — Lounas RSS-generaattori v6
- Antell: suora HTTP-haku
- Soupster: Browserless.io API (JavaScript-renderöinti pilvipalvelimella)
"""
...
# ── Browserless (Soupster) ────────────────────────────────────────────────────

def scrape_soupster_browserless(url: str, today_fi: str) -> str:
    """Hakee Soupster-sivun Browserless.io:n kautta joka renderöi JS:n."""
    api_key = os.environ.get("BROWSERLESS_API_KEY", "")
    if not api_key:
        return "(BROWSERLESS_API_KEY puuttuu ympäristömuuttujista)"

    # Browserless /content endpoint: renderöi sivun ja palauttaa HTML:n
    endpoint = f"https://production-sfo.browserless.io/content?token={api_key}"

    payload = json.dumps({
        "url": url,
        "waitForTimeout": 5000,   # odota 5s JS-renderöintiä
        "gotoOptions": {
    ...
    # Debug: tulosta tekstiä lokkiin
    text_preview = re.sub(r'<[^>]+>', ' ', html)
    text_preview = re.sub(r'\s+', ' ', text_preview).strip()
    print(f"\n    Browserless-esikatselu ({len(html)} merkkiä):\n    {text_preview[:500]}")

    return parse_soupster_html(html, today_fi)


def parse_soupster_html(html: str, today_fi: str) -> str:
    """Poimii päivän menun Soupsterin HTML:stä."""
    html = re.sub(r'<script[^>]*>.*?</script>', '', html, flags=re.DOTALL)
    html = re.sub(r'<style[^>]*>.*?</style>',  '', html, flags=re.DOTALL)
    text = re.sub(r'<[^>]+>', '\n', html)
    lines = [l.strip() for l in text.splitlines() if l.strip()]

    # Soupster käyttää päivien lyhenteitä: Ma, Ti, Ke, To, Pe
    paiva_lyhenne = {"maanantai": "Ma", "tiistai": "Ti", "keskiviikko": "Ke",
                     "torstai": "To", "perjantai": "Pe"}
    muut_lyhenteet = [v for k, v in paiva_lyhenne.items() if k != today_fi]
    ...
    for line in lines:
        if re.match(rf'^{today_short}\b', line) and not capturing:
            ...
            continue
        if capturing:
            if any(re.match(rf'^{d}\b', line) for d in muut_lyhenteet):
                break
    ...
    # Fallback: etsi "lounas"-sanan jälkeen tuleva sisältö
    ...
    for r in RESTAURANTS:
        ...
        try:
            if r["type"] == "antell":
                ...
                menu = scrape_soupster_browserless(r["url"], today_fi)
            print("OK")
